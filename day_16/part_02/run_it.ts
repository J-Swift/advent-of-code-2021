...
function getValue(p: Packet): number {
    if (isLiteral(p)) return p.payload.number

    let subValues = p.payload.subpackets.map(getValue)
    switch(p.typeId) {
        // sum
        case 0:
            return subValues.reduce((memo, val) => memo+val, 0)
        // product
        case 1:
            return subValues.reduce((memo, val) => memo*val, 1)
        // minimum
        case 2:
            return subValues.reduce((memo, val) => val < memo ? val : memo, subValues[0])
        // maximum
        case 3:
            return subValues.reduce((memo, val) => val > memo ? val : memo, subValues[0])
        // greater than
        ...
        case 7:
            return subValues[0] == subValues[1] ? 1 : 0
        default:
            throw `Unknown typeid for packet [${p.typeId}]`
    }
}
