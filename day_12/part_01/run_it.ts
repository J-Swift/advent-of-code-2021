...
const isSmallCave = (c: Cave) => 'a'.charCodeAt(0) <= c.value.charCodeAt(0) && c.value.charCodeAt(0) <= 'z'.charCodeAt(0)
const isLargeCave = (c: Cave) => 'A'.charCodeAt(0) <= c.value.charCodeAt(0) && c.value.charCodeAt(0) <= 'Z'.charCodeAt(0)
...
function getPathsToEnd(from: Cave, visited: Set<Cave>, paths: string[][]): string[][] {
    if (isEnd(from)) {
        return paths
    }
    ...
    return Array.from(from.connections).map(conn => {
        ...
        let newPaths = getPathsToEnd(conn, newVisited, paths.map(it => [...it, conn.value])).filter(it => it.length > 0)
        return newPaths
